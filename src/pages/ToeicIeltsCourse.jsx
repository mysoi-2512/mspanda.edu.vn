import { ToeicIelts } from "../constants";
import RegisterForm from "../components/RegisterForm";
import OtherCourses from "../components/OtherCourses";

const ToeicIeltsCourse = () => {
  return (
    <div className="relative max-container padding">
        <div className="flex flex-col items-center">
            <h3 className="text-xl text-center font-bold bg-main-yellow text-main-brown">{ToeicIelts.category}</h3>
            <h2 className='text-4xl text-center capitalize font-bold  text-main-brown mt-5'>{ToeicIelts.name}</h2>
        </div>
        <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-1 mt-20 mb-20 gap-5 justify-center px-10">
            {ToeicIelts.object.map((item, index) => (
        ...
        <div className="flex justify-between items-center max-lg:flex-col gap-5 px-20 max-lg:px-5">
            <div className="flex-1">
                ...
                {ToeicIelts.info.map((item, index) => (
            ...
            <div className="flex-1">
                ...
                {ToeicIelts.path.map((item, index) => (
                    <ul className="marker:text-main-brown list-disc px-5 mt-5">
                        <li key={index}>
        ...
        <div className="w-full mt-20 mb-20 ">
            <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 sm:gap-6 gap-14">
                {ToeicIelts.images.map((image, index) => (
                    <img 
                        src={image.imgURL} 
                        alt={image.alt} 
                        key={index}
        ...
        <div className=" mt-20 mb-20">
            ...
            <OtherCourses courseName="Luyện thi TOEIC/ IELTS"/>
        </div>        
    </div>
  )
}

export default ToeicIeltsCourse;
