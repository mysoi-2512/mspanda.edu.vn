import { Cambridge } from "../constants";
import RegisterForm from "../components/RegisterForm";
import OtherCourses from "../components/OtherCourses";

const CambridgeCource = () => {
  return (
    <div className="relative max-container padding">
        <div className="flex flex-col items-center">
            <h3 className="text-xl text-center font-bold bg-main-yellow text-main-brown">{Cambridge.category}</h3>
            <h2 className='text-4xl text-center capitalize font-bold lg:max-w-lg text-main-brown mt-5'>{Cambridge.name}</h2>
        </div>
        <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-1 mt-20 mb-20 gap-5 justify-center px-10">
            {Cambridge.object.map((item, index) => (
        ...
        <div className="w-full mt-20 mb-20 ">
            <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 sm:gap-6 gap-14">
                {Cambridge.images.map((image, index) => (
                    <img 
                        src={image.imgURL} 
                        alt={image.alt} 
                        key={index}
        ...
        <div className=" mt-20 mb-20">
            ...
            <OtherCourses courseName="Luyện thi Cambridge"/>
        </div>        
    </div>
  )
}

export default CambridgeCource;
